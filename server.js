...
function runFFmpeg(args, cwd) {
  ...
  return new Promise((resolve, reject) => {
    ...
    p.on("close", code => {
      ...
      return code === 0 ? resolve(log) : reject(new Error(log));
    });
  });
}
...
app.post("/probe", async (_req, res) => {
  try {
    res.json({ ok: true, out: await runFFmpeg(["-version"]) });
  } catch (e) {
...
app.post("/render", async (req, res) => {
  ...
  if (!voiceover)
    return res.status(400).json({ ok: false, error: "voiceover required" });
  ...
  try {
    ...
    for (let i = 0; i < images.length; i++) {
      const p = path.join(tmp, `img-${String(i).padStart(3, "0")}.jpg`);
      await downloadTo(p, images[i]);
      localImgs.push(p);
    }

    const voice = path.join(tmp, "voice.mp3");
    await downloadTo(voice, voiceover);
    let subs = "";
    if (subtitles) {
      subs = path.join(tmp, "subs.srt");
      await downloadTo(subs, subtitles);
    }

    const listTxt = path.join(tmp, "list.txt");
    let txt = "";
    for (const img of localImgs) {
      txt += `file '${img.replace(/'/g, "'\\''")}'\n`;
    ...
    const vf = subtitles
      ? `${vfBase},subtitles='${subs.replace(/\\/g, "\\\\").replace(/:/g, "\\:")}'`
      : vfBase;

    const args = [
      "-y", "-f", "concat", "-safe", "0", "-i", listTxt, "-r", String(fps),
      "-i", voice, "-vf", vf, "-c:v", "libx264", "-preset", "veryfast",
